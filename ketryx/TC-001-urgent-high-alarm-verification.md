...
itemTests:
  - SRS-001
---
# TC-001 — Urgent high blood glucose alarm verification

**Preconditions:**
- A user profile is configured with the urgent-high alarm enabled and a known urgent-high threshold value.
...
**Steps:**
1. Inject a blood glucose entry whose value is one unit below the configured urgent-high threshold and whose timestamp is the current evaluation time.
2. Trigger an alarm evaluation cycle and observe whether an URGENT alarm is emitted on the WebSocket alarm channel.
3. Inject a second blood glucose entry whose value is one unit above the configured urgent-high threshold and whose timestamp is the current evaluation time.
